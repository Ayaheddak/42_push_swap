...
/*   Updated: 2022/06/20 02:23:00 by aheddak          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	rotate_elem(t_list *a, t_list *b, t_incs *incs)
{
	...
	else if (incs->r != 0 && a->head
		&& (a->head->data == incs->arr[incs->index]))
	{
...
void	utile_send_b_to_a(t_list *a, t_list *b, t_incs *incs)
{
	int	pos;
...
void	cond_a(t_list *a, t_list *b, t_incs *incs, t_args *args)
{
	...
	else if (a->top->data >= incs->arr[args->first]
		&& a->top->data <= incs->arr[args->last])
	{
